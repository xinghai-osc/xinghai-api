...
func GetChannel(group string, model string, retry int, requestPath string) (*Channel, error) {
	return GetChannelWithCondition(group, model, retry, requestPath, nil)
}

func GetChannelWithCondition(group string, model string, retry int, requestPath string, condition func(*Channel) bool) (*Channel, error) {
	var abilities []Ability
	...
	if len(abilities) > 0 {
		...
		for _, ability_ := range abilities {
			...
			if condition == nil || condition(candidate) {
				filteredAbilities = append(filteredAbilities, ability_)
			}
		}
