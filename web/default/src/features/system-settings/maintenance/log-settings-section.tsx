...
export function LogSettingsSection({
  ...
  const handleCleanLogs = async () => {
    ...
    try {
      if (purgeScope === 'error') {
        // Error-only deletion uses the legacy sync endpoint; the async system
        // task only supports deleting all logs.
        const res = await deleteLogsBefore(purgeTimestamp, 'error')
        if (!res.success) {
